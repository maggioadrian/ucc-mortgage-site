...
function FarmBackdrop() {
  return (
    ...
    >
      <defs>
        <style>{`
          @keyframes farm-drift {
            0%   { transform: translateY(0); }
            100% { transform: translateY(-40px); }
          }
          .farm-rows { animation: farm-drift 18s ease-in-out infinite alternate; }
        `}</style>
      </defs>
      <g className="farm-rows" fill="none" stroke="#006f7f" strokeOpacity="0.08">
        {/* perspective crop rows — converging to vanishing point */}
        {Array.from({ length: 14 }).map((_, i) => {
          const yStart = 320 + i * 24;
          const spread = i * 55;
          return (
            <path
              key={i}
              d={`M ${720 - spread} ${yStart} Q 720 ${yStart - 60} ${720 + spread} ${yStart}`}
              strokeWidth={1 + i * 0.15}
            />
          );
        })}
        {/* wheat stalk accents */}
        {[160, 340, 520, 900, 1080, 1280].map((x, i) => (
          <g key={i} transform={`translate(${x}, 360)`} strokeOpacity="0.1">
            <line x1="0" y1="0" x2="0" y2="-80" strokeWidth="1.5" />
            <line x1="0" y1="-60" x2="-14" y2="-80" strokeWidth="1" />
            <line x1="0" y1="-60" x2="14" y2="-80" strokeWidth="1" />
            <line x1="0" y1="-45" x2="-10" y2="-58" strokeWidth="1" />
            <line x1="0" y1="-45" x2="10" y2="-58" strokeWidth="1" />
          </g>
        ))}
      </g>
      {/* horizon line */}
      <line x1="0" y1="320" x2="1440" y2="320" stroke="#006f7f" strokeOpacity="0.06" strokeWidth="1" />
    </svg>
  );
}
...
function ResidentialBackdrop() {
  return (
    ...
    >
      <defs>
        <style>{`
          @keyframes res-pulse-1 { 0%,100% { opacity:0.08; } 50% { opacity:0.14; } }
          @keyframes res-pulse-2 { 0%,100% { opacity:0.05; } 50% { opacity:0.10; } }
          @keyframes res-pulse-3 { 0%,100% { opacity:0.04; } 50% { opacity:0.09; } }
      ...
      {/* house roofline silhouettes — staggered opacity pulse */}
...
function CommercialBackdrop() {
  return (
    ...
    >
      <defs>
        <style>{`
          @keyframes com-blink-a { 0%,100% { opacity:0; } 48%,52% { opacity:0.18; } }
          @keyframes com-blink-b { 0%,100% { opacity:0; } 28%,32% { opacity:0.12; } }
          @keyframes com-blink-c { 0%,100% { opacity:0; } 68%,72% { opacity:0.14; } }
      ...
      {/* city grid — building outlines */}
      <g fill="none" stroke="#27aae1" strokeOpacity="0.06" strokeWidth="1">
        {/* tall towers */}
        ...
        {[220, 260, 300, 340].map((y) => <line key={y} x1="200" y1={y} x2="280" y2={y} strokeOpacity="0.04" />)}
        {[240, 280, 320].map((y) => <line key={y} x1="560" y1={y} x2="660" y2={y} strokeOpacity="0.04" />)}
      </g>
      {/* blinking windows */}
      <g fill="#27aae1">
...
function PrivateBackdrop() {
  ...
  return (
    ...
    >
      ...
      <g className="priv-hex-group" fill="none" stroke="#2e5f92" strokeOpacity="0.07" strokeWidth="1">
        {hexGrid.map(([cx, cy], i) => (
          <polygon key={i} points={hexPoints(cx, cy, 46)} />
        ))}
        {/* shield overlay at centre */}
        <path
          d="M720 180 L780 210 L780 300 Q780 360 720 390 Q660 360 660 300 L660 210 Z"
...
function DebtConsolidationBackdrop() {
  // Lines converging from edges toward centre-right
  const lines = [
  ...
  return (
    ...
    >
      <defs>
        <style>{`
          @keyframes dc-draw {
            0%   { stroke-dashoffset: ${totalLen}; opacity: 0.04; }
            40%  { opacity: 0.12; }
            100% { stroke-dashoffset: 0; opacity: 0.08; }
          }
          @keyframes dc-pulse { 0%,100% { r: 5; opacity: 0.12; } 50% { r: 9; opacity: 0.22; } }
      ...
      {/* origin dots */}
      {lines.map((l, i) => (
        <circle key={i} cx={l.x1} cy={l.y1} r="3" fill="#006f7f" fillOpacity="0.12" />
      ))}
    </svg>
  );
}
    ...
    >
      ...
      <g fill="none" stroke="#27aae1">
        ...
        <circle className="ref-arc1" cx="720" cy="300" r="340"
          strokeOpacity="0.06" strokeWidth="1.5"
          strokeDasharray="30 18" />
        {/* mid arc */}
        <circle className="ref-arc2" cx="720" cy="300" r="240"
          strokeOpacity="0.05" strokeWidth="1.5"
          strokeDasharray="20 30" />
        {/* inner solid */}
        <circle className="ref-arc3" cx="720" cy="300" r="150"
          strokeOpacity="0.07" strokeWidth="1.5"
          strokeDasharray="12 20" />
        {/* static radial tick marks */}
        {Array.from({ length: 12 }).map((_, i) => {
          ...
          return (
            <line key={i}
              ...
              stroke="#27aae1" strokeOpacity="0.08" strokeWidth="1"
            />
          );
        })}
      </g>
...
function LoansBackdrop() {
  ...
  const buildPath = (fn: (x: number) => number) => {
    const pts = Array.from({ length: 145 }, (_, i) => {
      const x = i * 10;
      const y = fn(x);
      return `${i === 0 ? "M" : "L"} ${x} ${y}`;
    });
  ...
  return (
    ...
    >
      ...
      <g fill="none" stroke="#006f7f">
        <path className="loans-w1"
          d={buildPath((x) => wave1(x, 0))}
          strokeOpacity="0.07" strokeWidth="1.5" />
        <path className="loans-w1"
          d={buildPath((x) => wave1(x, Math.PI))}
          strokeOpacity="0.04" strokeWidth="1" />
        <path className="loans-w2"
          d={buildPath((x) => wave2(x, 0.8))}
          strokeOpacity="0.06" strokeWidth="1.5" />
        <path className="loans-w2"
          d={buildPath((x) => wave2(x, 0.8 + Math.PI))}
          strokeOpacity="0.04" strokeWidth="1" />
      </g>
    </svg>
  );
...
function RatesBackdrop() {
  ...
  return (
    ...
    >
      <defs>
        <style>{`
          @keyframes rate-scroll { 0%,100% { opacity:0.04; } 50% { opacity:0.09; } }
...
function ApplyBackdrop() {
  ...
  return (
    ...
    >
      <defs>
        <style>{`
          @keyframes apply-dash {
            0%   { stroke-dashoffset: 800; }
            100% { stroke-dashoffset: 0; }
          }
          @keyframes apply-pulse { 0%,100% { r: 6; opacity: 0.1; } 50% { r: 11; opacity: 0.2; } }
          @keyframes apply-pulse-outer { 0%,100% { r: 14; opacity: 0.04; } 50% { r: 20; opacity: 0.1; } }
      ...
      <path
        className="apply-path"
        d={`M ${nodePositions[0]} ${y} ${nodePositions.slice(1).map((x) => `L ${x} ${y}`).join(" ")}`}
        fill="none" stroke="#006f7f" strokeOpacity="0.12" strokeWidth="2"
      />
      {/* pulsing nodes */}
      ...
      {["Start", "Details", "Property", "Income", "Review", "Done"].map((label, i) => (
        <text key={i} x={nodePositions[i]} y={y + 36} textAnchor="middle"
          fill="#006f7f" fillOpacity="0.09" fontSize="11"
...
function AboutBackdrop() {
  return (
    ...
    >
      ...
      {/* Windsor Ambassador Bridge + city skyline silhouette */}
      <g className="about-skyline" fill="none" stroke="#2e5f92" strokeOpacity="0.06" strokeWidth="1.2">
        {/* Ambassador Bridge cables */}
        ...
        {/* cable stays left tower */}
        {Array.from({ length: 8 }).map((_, i) => (
          <line key={`cl${i}`} x1="420" y1={130 + i * 18} x2={420 - 30 - i * 28} y2="440" strokeOpacity="0.04" />
        ))}
        {/* cable stays right tower */}
        {Array.from({ length: 8 }).map((_, i) => (
          <line key={`cr${i}`} x1="760" y1={130 + i * 18} x2={760 + 30 + i * 28} y2="440" strokeOpacity="0.04" />
        ))}
        {/* Windsor city buildings */}
        <rect x="60" y="340" width="40" height="100" />
        <rect x="110" y="300" width="55" height="140" />
        <rect x="175" y="360" width="35" height="80" />
        {/* Detroit skyline (far shore) */}
        <rect x="900" y="380" width="30" height="60" strokeOpacity="0.04" />
        <rect x="940" y="350" width="50" height="90" strokeOpacity="0.04" />
        <rect x="1000" y="340" width="40" height="100" strokeOpacity="0.04" />
        <rect x="1050" y="370" width="35" height="70" strokeOpacity="0.04" />
        <rect x="1100" y="360" width="60" height="80" strokeOpacity="0.04" />
        <rect x="1170" y="380" width="30" height="60" strokeOpacity="0.04" />
        {/* river */}
        <line x1="0" y1="440" x2="1440" y2="440" strokeOpacity="0.08" />
      </g>
    </svg>
  );
}
    ...
    >
      <defs>
        <style>{`
          ...
          @keyframes inv-dot { 0%,100% { r: 4; opacity: 0.12; } 50% { r: 7; opacity: 0.22; } }
      ...
      {[160, 240, 320, 400, 480].map((y) => (
        <line key={y} x1="150" y1={y} x2="1290" y2={y}
          stroke="#27aae1" strokeOpacity="0.03" strokeWidth="1" />
      ))}
      {/* yield curve */}
      <path className="inv-curve" d={pts}
        fill="none" stroke="#27aae1" strokeOpacity="0.1" strokeWidth="1.5" />
      {/* data points */}
      {dataPoints.map((pt, i) => (
      ...
      <line x1="200" y1="160" x2="200" y2="490"
        stroke="#27aae1" strokeOpacity="0.06" strokeWidth="1" />
  ...
  return (
    ...
    >
      ...
      <g className="land-contours" fill="none" stroke="#006f7f">
        {contours.map((d, i) => (
          <path key={i} d={d} strokeOpacity={0.05 + i * 0.01} strokeWidth="1" />
        ))}
        {/* horizon */}
        <line x1="0" y1="200" x2="1440" y2="200" strokeOpacity="0.06" strokeWidth="1.2" />
        {/* sun */}
        <circle cx="1100" cy="160" r="36" strokeOpacity="0.05" strokeWidth="1.5" fill="none" />
        <circle cx="1100" cy="160" r="22" strokeOpacity="0.04" strokeWidth="1" fill="none" />
      </g>
    </svg>
  );
}
